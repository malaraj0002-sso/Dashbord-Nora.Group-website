...
import { GlassCard, SectionHeader, ImageUpload, FieldLabel, AddButton, DeleteButton, ReorderButtons, ItemRow, PublishToggle, StatusBadge } from '../shared';
import { LocalizedInput, LanguageTabs } from '../LanguageTabs';
import { Input } from '@/components/ui/input';

export function HeroModule() {
  const { data, addHeroSlide, updateHeroSlide, deleteHeroSlide, reorderHeroSlides } = useAdminData();
      ...
      <div className="space-y-4">
            ...
            <ItemRow key={slide.id}>
              <div className="flex gap-4">
                <div className="flex flex-col items-center justify-center gap-1">
                  <ReorderButtons onUp={() => reorderHeroSlides(slide.id, 'up')} onDown={() => reorderHeroSlides(slide.id, 'down')} />
                </div>
                ...
                <div className="flex-1 space-y-3">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-medium text-gold">Slide #{slide.order + 1}</span>
                    <div className="flex items-center gap-3">
                      <PublishToggle
                        published={slide.published}
                        onChange={(v) => updateHeroSlide(slide.id, { published: v })}
                      />
                      <DeleteButton onClick={() => deleteHeroSlide(slide.id)} />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-1 gap-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <LocalizedInput
                        value={slide.ctaText}
                        onChange={(v) => updateHeroSlide(slide.id, { ctaText: v })}
        ...
        {heroSlides.length === 0 && (
          <GlassCard className="text-center py-12">
            ...
            <p className="text-sm text-muted-foreground">No hero slides yet. Click "Add Slide" to create one.</p>
          </GlassCard>
        )}
      </div>
    </div>
